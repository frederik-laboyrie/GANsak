...
def generator_unet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        d2 = resizeconv2d(tf.nn.relu(d1), int(e6.shape[1]), options.gf_dim*8*2, name='resize_2') if resize else \
        ...
        d3 = resizeconv2d(tf.nn.relu(d2), int(e5.shape[1]), options.gf_dim*8*2, name='resize_3') if resize else \
            deconv2d(tf.nn.relu(d2), options.gf_dim*8, name='g_d3')
        d3 = tf.nn.dropout(d3, dropout_rate)
        d3 = tf.concat([instance_norm(d3, 'g_bn_d3'), e5], 3)
        # d3 is (8 x 8 x self.gf_dim*8*2)

        d4 = resizeconv2d(tf.nn.relu(d3), int(e4.shape[1]), options.gf_dim*8*2, name='resize_4') if resize else \
            deconv2d(tf.nn.relu(d3), options.gf_dim*8, name='g_d4')
        d4 = tf.concat([instance_norm(d4, 'g_bn_d4'), e4], 3)
        # d4 is (16 x 16 x self.gf_dim*8*2)

        d5 = resizeconv2d(tf.nn.relu(d4), int(e3.shape[1]), options.gf_dim*4*2, name='resize_6') if resize else \
            deconv2d(tf.nn.relu(d4), options.gf_dim*4, name='g_d5')
        d5 = tf.concat([instance_norm(d5, 'g_bn_d5'), e3], 3)
        # d5 is (32 x 32 x self.gf_dim*4*2)

        d6 = resizeconv2d(tf.nn.relu(d5), int(e2.shape[1]), options.gf_dim*2*2, name='resize_7') if resize else \
            deconv2d(tf.nn.relu(d5), options.gf_dim*2, name='g_d6')
        tf.concat([instance_norm(d6, 'g_bn_d6'), e2], 3)
        # d6 is (64 x 64 x self.gf_dim*2*2)

        d7 = resizeconv2d(tf.nn.relu(d6), int(e1.shape[1]), options.gf_dim*1*2, name='resize_8') if resize else \
            deconv2d(tf.nn.relu(d6), options.gf_dim, name='g_d7')
        d7 = tf.concat([instance_norm(d7, 'g_bn_d7'), e1], 3)
        # d7 is (128 x 128 x self.gf_dim*1*2)

        d8 = resizeconv2d(tf.nn.relu(d7), options.image_size, options.output_c_dim, name='resize_9') if resize else \
